...
class ProphetARIMAEnsemble():
    ...
    def __str__(self):
        return f'ProphetARIMAEnsemble(order={self.order}, ' \
            + f'seasonal_order={self.seasonal_order}, prophet_default_alpha={self.alpha})'
    ...
    def predict(self, horizon, alpha=0.05, return_all_models=False):
        ...
        summary_frame['yhat'] = summary_frame[['prophet_mean', 'arima_mean']].mean(axis=1)
        summary_frame['yhat_lower_' + post_fix] = \
            summary_frame[['prophet_lower_' + post_fix, 'arima_lower_' + post_fix]].mean(axis=1)
        summary_frame['yhat_upper' + post_fix] = \
            summary_frame[['prophet_upper_' + post_fix, 'arima_upper_' + post_fix]].mean(axis=1)
    ...
    def _arima_predict(self, horizon, alpha=0.05):
        ...
        post_fix = str(int((1 - alpha) * 100))
        df.columns = ['arima_mean', 'arima_lower_' + post_fix, 'arima_upper_' + post_fix]
        df.index.name = 'ds'
        return df
    ...
    def _prophet_predict(self, horizon, alpha):
        ...
        df = prophet_forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']][-horizon:]
        post_fix = str(int((1 - alpha) * 100))
        df.columns = ['ds', 'prophet_mean', 'prophet_lower_' + post_fix, 'prophet_upper_' + post_fix]
        df.index = df['ds']
    ...
    def check_is_fitted(self):
        if not self._fitted:
            msg = 'Ensemble model has not been fitted. Please call model.fit(y_train)'
